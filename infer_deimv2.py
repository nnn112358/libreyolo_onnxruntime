...
# ///
"""DEIMv2 ONNX 推論 (standalone, NMSフリー)。

family=deimv2。出力 pred_logits[1,Q,nc] + pred_boxes[1,Q,4](cxcywh,[0,1])。
前処理: RGB / 単純square リサイズ / 255 / CHW。
  注意: DEIMv2 の DINO バックボーン採用サイズ(s/m/l/x)は ImageNet正規化が必要。
  本スクリプトの既定 atto は非DINOなので /255 のみ(--imagenet で正規化を有効化可)。
後処理は D-FINE と同じDETR集合予測(NMS不要)。

  uv run infer_deimv2.py
  uv run infer_deimv2.py --model deimv2_s.onnx --imagenet   # DINOサイズの場合
"""
...
def main():
    ...
    ap.add_argument("--model", default=os.path.join(HERE, "deimv2_atto.onnx"))
    ap.add_argument("--image", default=os.path.join(HERE, "sample_640x480.jpg"))
    ap.add_argument("--out", default=None)
    ap.add_argument("--conf", type=float, default=0.25)
    ap.add_argument("--imagenet", action="store_true", help="ImageNet正規化(DINOサイズ s/m/l/x 用)")
    a = ap.parse_args()
    out_path = a.out or os.path.splitext(a.image)[0] + "_deimv2.jpg"
    ...
    print(f"[deimv2] model={os.path.basename(a.model)} imgsz={imgsz} image={W}x{H} detected={len(dets)} (NMSフリー)")
