...
# ///
"""RT-DETRv4 ONNX 推論 (standalone, NMSフリー)。

family=rtdetrv4。出力 pred_logits[1,Q,nc] + pred_boxes[1,Q,4](cxcywh,[0,1])。
前処理: RGB / 単純square リサイズ / 255 / CHW。後処理は D-FINE と同じDETR集合予測。
...
def main():
    ...
    ap.add_argument("--model", default=os.path.join(HERE, "rtdetrv4_s.onnx"))
    ap.add_argument("--image", default=os.path.join(HERE, "image", "sample_640x480.jpg"))
    ap.add_argument("--out", default=None)
    ap.add_argument("--conf", type=float, default=0.25)
    a = ap.parse_args()
    out_path = a.out or os.path.splitext(a.image)[0] + "_rtdetrv4.jpg"
    ...
    print(f"[rtdetrv4] model={os.path.basename(a.model)} imgsz={imgsz} image={W}x{H} detected={len(dets)} (NMSフリー)")
